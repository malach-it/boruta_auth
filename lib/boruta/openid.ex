...
defmodule Boruta.Openid do
  ...
  def direct_post(conn, direct_post_params, module) do
    with {:ok, _claims} <- check_id_token_client(direct_post_params),
      ...
      with {:ok, code} <-
           ...
           :ok <- maybe_check_public_client_id(direct_post_params, code.public_client_id, code.client),
  ...
  defp maybe_check_public_client_id(_direct_post_params, _public_client_id, %Client{check_public_client_id: false}), do: :ok

  defp maybe_check_public_client_id(%{id_token: id_token}, "did:" <> _key = public_client_id, _client) do
    with {:ok, %{"alg" => alg}} <- Joken.peek_header(id_token),
         {:ok, _jwk, _claims} <- VerifiablePresentations.verify_jwt({:did, public_client_id}, alg, id_token) do
      :ok
    else
      {:error, _error} ->
        {:error, %Error{
  ...
  defp maybe_check_public_client_id(%{vp_token: vp_token}, "did:" <> _key = public_client_id, _client) do
    with {:ok, %{"alg" => alg}} <- Joken.peek_header(vp_token),
         {:ok, _jwk, _claims} <- VerifiablePresentations.verify_jwt({:did, public_client_id}, alg, vp_token) do
      :ok
    else
      {:error, _error} ->
        {:error, %Error{
  ...
  defp maybe_check_public_client_id(_direct_post_params, _public_client_id, _client), do: :ok
