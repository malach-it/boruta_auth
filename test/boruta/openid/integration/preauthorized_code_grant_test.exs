  ...
  describe "authorization code grant - authorize" do
    ...
    test "returns an error if `response_type` is preauthorized and schema is invalid" do
      assert Oauth.authorize(
               %Plug.Conn{query_params: %{"response_type" => "urn:ietf:params:oauth:response-type:pre-authorized_code"}},
...
#     end
    ...
    test "returns a credential offer response", %{client: client, resource_owner: resource_owner} do
      redirect_uri = List.first(client.redirect_uris)
      resource_owner = %{resource_owner | available_credentials: ["credential"]}
...
#                     "Required properties code, client_id, redirect_uri are missing at #.",
#                   status: :bad_request
#                 }}
#     end
#
