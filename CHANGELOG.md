...
- Note that you must run the `boruta.gen.migration` task to rename the primary key constraints.

### Fixed

- primary keys and the `oauth_clients_scopes` sequence carry the `oauth_` prefixed names; duplicate client and scope ids are reported as changeset errors whichever name the constraint has

## [2.3.8] - 2026-08-01

### Fixed

- trusted hosts / authorities default to empty values instead of nil
...
- update dependencies (Nebulex major version upgrade)

## [2.3.5] - 2025-07-05

### Fixed

- use clients context from dependency injection to resolve token structures
